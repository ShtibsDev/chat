...
const MultiSelector = ({ values: value, onValuesChange: onValueChange, loop = false, className, children, dir, ...props }: MultiSelectorProps) => {
	...
	return (
		<MultiSelectContext.Provider
			...
			<Command onKeyDown={handleKeyDown} className={cn('overflow-visible bg-transparent flex flex-col space-y-2', className)} dir={dir} {...props}>
				{children}
			</Command>
		</MultiSelectContext.Provider>
	);
...
const MultiSelectorTrigger = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { render?: (value: string) => ReactNode }>(
	({ className, children, render, ...props }, ref) => {
		...
		return (
			<div ref={ref} className={cn('flex flex-wrap gap-1 p-1 py-2 border border-muted rounded-lg bg-background', className)} {...props}>
				{value.map((item, index) => (
					<Badge
						key={item}
						className={cn('px-1 rounded-xl flex items-center gap-1', activeIndex === index && 'ring-2 ring-muted-foreground ')}
						variant={'secondary'}>
						<span className='text-xs'>{render ? render(item) : item}</span>
						<button
